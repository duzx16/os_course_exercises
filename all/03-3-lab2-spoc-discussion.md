...
3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   ...
   中断响应的时候，要求CPL<=DPL(门)且CPL>=DPL(段)

   函数调用和指令执行的时候，要求max(CPL, RPL)<=DPL
...
我调用的是int 0x6. 加入之后在intr_enable之后，kernel会因为触发了unexpected trap而panic(在trap_dispatch中只进行了有限几种中断的处理，其他的都触发了panic)。之后会进入kernel debug monitor。
...
1. (midd) 在x86-32中，假设页目录的第R个页表项是自映射的，那么：
   ...
   2. 控制虚地址`vaddr`所在页的页表项地址是多少？（假设在`vaddr`发生了缺页异常，根据你给出的答案，就可以快速地修改相应的页表项）
      ...
      (R-1) * 4G / 1024 + vaddr[31:22] * 4K + vaddr[21:12] * 4
